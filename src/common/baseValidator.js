const formValidate = {
	...
	validateEmail:(rule,value,callback)=>{
		...
		}else	if (/^([a-z0-9A-Z]+[-|_\.]?)+[a-z0-9A-Z]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\.)+[a-zA-Z]{2,}$/.test(newValue) == false) {
	...
	validateLicense:(rule, value, callback) =>{
		...
		if(newValue.length == 0) {
			callback(new Error('请输入SN(字母或数字组成的2~20位字符)'));
		}else if(!reg.test(newValue)) {
			callback(new Error('SN必须是字母或数字组成的2~20位字符'));
		}else {
			callback();
		}
	},
