<template>
	<div class="timingBox">
        <div class="top">
            <div>
                <span class="backMain" @click="backOpt">定时控制</span>
			...
			<el-table :data="tableData" max-height="660" border empty-text="暂无数据" style="width: 100%">
				...
				<el-table-column prop="onOff" align="center" label="执行操作" width="250">
					<template slot-scope="scope">
						<el-tag :type="scope.row.onOff == 0?'success':'warning'" effect="dark">{{scope.row.onOff == 1?'开灯':scope.row.onOff == 0?'关灯':'调光'}}</el-tag>
					</template>
				</el-table-column>
